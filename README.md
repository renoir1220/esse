...
Windows Agent Sidecar 的签名发行产物遵循仓库公开的 [Code signing policy](CODE_SIGNING.md)。发布工作流通常会同时验证应用程序和安装程序的 Authenticode 签名；SignPath Foundation 审批期间，维护者明确允许 `v0.3.0-alpha.2` 与 `v0.3.0` 未签名发布，并在 Release 中保留醒目警告。后续版本仍受签名门禁保护。

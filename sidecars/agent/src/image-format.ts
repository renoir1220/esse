...
export function detectImageFormat(bytes: Uint8Array): ImageFormat | undefined {
  ...
  if (buffer.length >= 4 && (buffer.subarray(0, 4).equals(Buffer.from([0x49, 0x49, 0x2a, 0x00]))
    || buffer.subarray(0, 4).equals(Buffer.from([0x4d, 0x4d, 0x00, 0x2a])))) return { mimeType: 'image/tiff', extension: 'tiff' };
...
export function decodeImageBase64(value: string): Buffer {
  ...
  for (let index = 0; index < value.length; index += 1) {
    ...
    const valid = (code >= 0x41 && code <= 0x5a)
      ...
      || (code >= 0x30 && code <= 0x39)
      || code === 0x2b || code === 0x2f;
    if (!valid || sawPadding) throw new Error('Provider returned invalid base64 image data.');
    meaningful += 1;
  }
  if (!meaningful || meaningful % 4 !== 0 || padding > 2) throw new Error('Provider returned invalid base64 image data.');
  const decodedLength = Math.floor(meaningful * 3 / 4) - padding;
  if (decodedLength > MAX_IMAGE_BYTES) throw new Error('Generated image exceeds the 60 MB image limit.');

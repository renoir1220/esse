...
5. Verify both `codex plugin marketplace list` and `codex plugin list` after installation.
6. Never ask the user to paste an API key into chat, a shell command, an environment variable, or a repository file.
7. After a successful install, tell the user to restart the Codex/ChatGPT desktop app, start a new task, and say `打开 Esse 设置`. Provider credentials and the default image model must be configured inside the Esse settings UI.

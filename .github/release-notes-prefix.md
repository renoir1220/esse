...
- 本 Alpha 的 Windows 与 macOS Agent Sidecar 产物未做发布者签名或 Apple 公证；macOS 应用已通过结构有效的 ad-hoc 签名校验，但这不代表发布者身份。Release 继续提供 SHA256 校验。Windows 可能显示未知发布者，macOS Gatekeeper 仍可能拒绝打开，请勿关闭系统安全机制。
...
- Windows and macOS Agent Sidecar artifacts in this Alpha are not publisher-signed or Apple-notarized. The macOS app passes structural ad-hoc signature verification, but that does not establish publisher identity. SHA256 checksums remain available. Windows may show an unknown-publisher warning, and macOS Gatekeeper may still reject the app; do not disable platform security.
